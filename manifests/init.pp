...
class bacula(
  ...
  # If we are a director or storage
  # and don't have a director and
  # storage packages defined,
  # manage the db specific package we need
  if $is_director or $is_storage {
    if (! $director_package) and (! $storage_package) {
      class { 'bacula::common':
        packages         => "bacula-${db_backend}",
        manage_db_tables => $manage_db_tables,
        db_backend       => $db_backend,
        db_user          => $db_user,
        db_password      => $db_password,
